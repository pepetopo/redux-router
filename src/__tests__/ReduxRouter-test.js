...
describe('<ReduxRouter>', () => {
  ...
  it('should accept React.Components for "RoutingContext" prop of ReduxRouter', () => {
    ...
    renderIntoDocument(
      <Provider store={store}>
        <ReduxRouter RoutingContext={RouterContext}>{routes}</ReduxRouter>
      </Provider>
    );
  ...
  it('should not accept non-React-components for "RoutingContext" prop of ReduxRouter', () => {
    ...
    const routingContextInvalidPropErrorMessage =
    ...
    expect(consoleErrorSpy.args[0][0]).to.contain(
      routingContextInvalidPropErrorMessage
    );
  ...
  describe('server-side rendering', () => {
    ...
    it('handles redirects', () => {
      ...
      store.dispatch(
        server.match('/redirect', (error, redirectLocation) => {
